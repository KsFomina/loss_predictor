...
_request_id передаётся клиентом и используется как ключ: повторный POST с тем же request_id вернёт ранее сохранённый результат, не вызывая модель повторно._
